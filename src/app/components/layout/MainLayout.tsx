...
export function MainLayout() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Desktop sidebar */}
      <aside className="hidden lg:fixed lg:inset-y-0 lg:left-0 lg:z-50 lg:block lg:w-64 lg:overflow-y-auto lg:bg-[#1351B4] lg:shadow-xl">
        <SidebarContent />
      </aside>
...
function getBreadcrumbs(pathname: string) {
  ...
  for (const seg of segments) {
    cur += `/${seg}`;
    if (labels[seg]) crumbs.push({ path: cur, label: labels[seg] });
